...
// first = true/false if code should be ran initially before next timer interval
// hits
...
GUI_control.prototype.interval_kill_all = function(keep_array) {
  ...
  for (var i = (this.interval_array.length - 1); i >= 0;
       i--) {  // reverse iteration
    var keep = false;
...
GUI_control.prototype.log = function(message) {
  ...
  $('div.wrapper', command_log)
      .append('<p>' + formattedDate + ' -- ' + message + '</p>');
...
GUI_control.prototype.tab_switch_cleanup = function(callback) {
  ENP.callbacks_cleanup();  // we don't care about any old data that might or
                            // might not arrive
  GUI.interval_kill_all();  // all intervals (mostly data pulling) needs to be
...
GUI_control.prototype.switchery = function() {
  $('.togglesmall').each(function(index, elem) {
    var switchery = new Switchery(
        elem, {size: 'small', color: '#ffbb00', secondaryColor: '#c4c4c4'});
  ...
  $('.toggle').each(function(index, elem) {
    var switchery =
        new Switchery(elem, {color: '#ffbb00', secondaryColor: '#c4c4c4'});
    $(elem).on('change', function(evt) {
      switchery.setPosition();
    });
...
GUI_control.prototype.content_ready =
    function(callback) {
  this.switchery();
  ...
  jQuery(document).ready(function($) {
    $('cf_tip').each(
        function() {  // Grab all ".cf_tip" elements, and for each...
          log(this);  // ...print out "this", which now refers to each ".cf_tip"
                      // DOM element
        });
